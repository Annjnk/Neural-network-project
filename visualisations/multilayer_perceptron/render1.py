...
def safe_normalize(x):
    """Safely normalize an array to [0,1]; avoids division by zero or NaNs."""
    mx = np.max(x)
...
# --- Layer positions (simplified for readability) ---
# Input layer
...
def create_layer(shape, offset_y):
    ...
    x = x.ravel().astype(float) - shape[1]//2  # cast to float
    z = z.ravel().astype(float) - shape[0]//2  # cast to float
    y = np.ones_like(x) * offset_y
    # Add small random jitter for nicer visualization
...
# Adjust indices for flattened positions
to_fc1 += len(in_x)
...
for i, label in enumerate(range(10)):
    mlab.text3d(x=-14.5 + i*3, y=80.5, z=-2, text=str(label))
...
def anim():
    for frame in tqdm(range(160)):  # smaller number for testing
        if frame % 16 == 0:
            i = frame // 16
        ...
        # Optional: save frames (commented out for testing)
        # mlab.savefig(f'frames/frame{frame:04d}.png')
